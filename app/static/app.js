...
function updateScoreResults(result, inputData) {
  ...
  document.getElementById("res-threshold").textContent = result.threshold_used.toFixed(2);
  document.getElementById("res-latency").textContent = `${result.latency_ms.toFixed(1)} ms`;
  document.getElementById("res-id").textContent = result.prediction_id.slice(0, 8);

  // 5. Contributing risk factors calculation
  renderKeyFactors(inputData, prob);
...
function renderKeyFactors(input, prob) {
...
function addRecentPrediction(result, input) {
  const item = {
    ...
    probability: `${(result.default_probability * 100).toFixed(1)}%`,
    rating: result.risk_tier,
...
async function loadModelMetadata() {
  try {
    ...
    document.getElementById("meta-name").textContent = data.model_name;
    document.getElementById("meta-version").textContent = data.model_version;
    document.getElementById("meta-threshold").textContent = data.decision_threshold.toFixed(2);
    document.getElementById("meta-status").textContent = data.status.toUpperCase();

    // Test metrics
    const test = data.test_metrics;
    if (test) {
      document.getElementById("metric-roc-auc").textContent = test.roc_auc.toFixed(3);
      document.getElementById("metric-recall").textContent = `${(test.recall * 100).toFixed(1)}%`;
      document.getElementById("metric-precision").textContent = `${(test.precision * 100).toFixed(1)}%`;
      document.getElementById("metric-f1").textContent = test.f1.toFixed(3);
      document.getElementById("metric-prauc").textContent = test.pr_auc.toFixed(3);
      document.getElementById("metric-brier").textContent = test.brier_score.toFixed(3);
    }
  } catch (err) {
    console.error(err);
...
function renderDriftTable(data) {
  ...
  for (const [feat, info] of Object.entries(features)) {
    ...
    tr.innerHTML = `
      ...
      <td>${info.test_type.toUpperCase()}</td>
      <td>${info.statistic.toFixed(4)}</td>
      <td>${info.p_value !== null && info.p_value !== undefined ? info.p_value.toFixed(4) : "—"}</td>
  ...
  if (badge) {
    badge.textContent = data.drift_detected ? "Drift Warning" : "Zero Drift";
    badge.className = `driver-tag ${data.drift_detected ? 'negative' : 'positive'}`;
  }
}
